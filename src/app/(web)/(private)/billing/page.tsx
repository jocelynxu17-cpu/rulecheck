...
import { ensureWorkspaceForRequest } from "@/lib/workspace/ensure-workspace";
import { RepairWorkspaceButton } from "@/components/workspace/RepairWorkspaceButton";
...
export default async function BillingPage({ searchParams }: BillingPageProps) {
  ...
  const ensure = await ensureWorkspaceForRequest();
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();

  const billing = user ? await getPrimaryWorkspaceBillingUiSnapshot(supabase, user.id) : null;

  const yymm = new Date().toISOString().slice(0, 7);
  const used = billing && billing.usage_month === yymm ? billing.units_used_month : 0;
  const quota = billing?.monthly_quota_units ?? 0;
  const remaining = billing ? Math.max(0, quota - used) : 0;

  const rawStatus = billing?.subscription_status ?? null;
  const statusLabel = subscriptionStatusLabelZh(rawStatus);
  const uiState = deriveBillingUiState(billing);
  const periodEndLabel = formatPeriodEndTaipei(billing?.current_period_end ?? null);
  const planLower = (billing?.plan ?? "free").toLowerCase();
  const showProInterestCard = planLower === "free" && uiState === "free";
  const providerLabel = billingProviderLabelZh(billing?.billing_provider ?? null);
    ...
    <div className="mx-auto max-w-4xl space-y-10">
      <div className="max-w-2xl space-y-3">
        ...
        {billing ? (
          <p className="mt-2 text-sm text-ink-secondary">
            目前工作區：<span className="font-semibold text-ink">{billing.workspaceName}</span>
            <Link href="/members" className="ml-2 font-medium text-ink underline-offset-4 hover:underline">
              切換或管理成員與額度
            </Link>
          </p>
        ) : null}
      ...
      {!ensure.ok ? (
        <Card className="border-amber-200/80 bg-amber-50/40">
          <CardHeader>
            <CardTitle className="text-base text-amber-950">無法載入工作區帳務</CardTitle>
            <CardDescription className="text-amber-900/85">
              方案與共用審查額度以工作區為準。若初始化失敗，請先修復工作區後再重新整理此頁。
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      {ensure.ok && !billing ? (
        <Card className="border-surface-border bg-canvas">
          <CardHeader>
            <CardTitle className="text-base">找不到工作區</CardTitle>
            <CardDescription>請建立預設工作區後再檢視帳務摘要。</CardDescription>
          </CardHeader>
          <CardContent>
            <RepairWorkspaceButton />
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge tone="blue">{billing?.plan === "pro" ? "Pro" : "Free"}</Badge>
          ...
          <CardContent className="space-y-3 text-sm text-ink-secondary">
            ...
            <div className="flex justify-between gap-4 border-b border-surface-border/80 pb-2">
              <span>本月已使用</span>
              <span className="font-semibold text-ink">{billing ? used : "—"}</span>
            </div>
            <div className="flex justify-between gap-4">
              <span>剩餘共用審查額度</span>
              <span className="font-semibold text-ink">{billing ? remaining : "—"}</span>
            </div>
            <p className="pt-1 text-xs leading-relaxed">
              審查點數為工作區月度共用審查額度，多帳號共用同一份；文字與圖片各計 1 點，PDF 依頁數計。調整上限請至「成員」頁面（擁有者／管理員）。
            </p>
          </CardContent>
        </Card>
