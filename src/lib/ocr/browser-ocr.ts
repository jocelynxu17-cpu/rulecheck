...
async function runOcrWithSingleWorker(
  ...
  try {
    ...
    return augmentOcrWithDisplayText({ ...best.detailed, browserPipeline, hanScript });
  } finally {
 ...
 * `text` 保留引擎聚合原文；`textDisplay`／行區塊為顯示層字形優化（見 `displayNormalization`）。`hanScript` 係對引擎原文之統計。
 */
